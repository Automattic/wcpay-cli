export function formatKeyValue( rows: Record<string, unknown> ): string {
...
export function formatList( data: unknown, columns: string[] ): string {
	...
	const widths = columns.map( ( column ) =>
		Math.max( column.length, ...items.map( ( item ) => formatValue( item[ column ] ).length ) )
	);
	const header = columns.map( ( column, index ) => column.padEnd( widths[ index ] ) ).join( '  ' );
	const divider = widths.map( ( width ) => '-'.repeat( width ) ).join( '  ' );
	const body = items
		.map( ( item ) =>
			columns.map( ( column, index ) => formatValue( item[ column ] ).padEnd( widths[ index ] ) ).join( '  ' )
		)
		.join( '\n' );
