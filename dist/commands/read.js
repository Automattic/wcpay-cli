import { WCPAY_ABILITIES, listAbilities, runReadAbilityWithRestFallback, } from '../core/abilities.js';
...
export function registerReadCommands(program) {
    ...
    program
        ...
        await runReadAction({ json }, async () => {
            ...
            checks.push(mode === 'live'
                ? warn('mode', 'live — write commands will be blocked')
                : pass('mode', mode));
            checks.push(settings.is_wcpay_enabled === false
            ...
            printSuccess({ profile: context.profile.name, site: context.profile.siteUrl, mode, summary, checks }, {
                json,
    ...
    account
        ...
        await runReadAction({ json }, async () => {
            const context = await createContext({ profile: program.opts().profile });
            const data = await runReadAbilityWithRestFallback(context.client, WCPAY_ABILITIES.getAccount, {}, () => context.client.request({ method: 'GET', path: '/wc/v3/payments/accounts' }));
            printSuccess(data, {
                json,
...
async function addAccountCheck(checks, context) {
    try {
        const account = await runReadAbilityWithRestFallback(context.client, WCPAY_ABILITIES.getAccount, {}, () => context.client.request({ method: 'GET', path: '/wc/v3/payments/accounts' }));
        if (account === false) {
        ...
        if (account && typeof account === 'object') {
            ...
            checks.push(pass('account', `status: ${String(accountRecord.status ?? 'unknown')}`, {
...
async function addBrowserLoginCheck(checks, context) {
    try {
...
async function addAbilitiesCheck(checks, context) {
    try {
...
function formatDoctorChecks(checks, summary) {
    const lines = [
...
function getDoctorNextSteps(checks) {
    const steps = [];
...
function formatAccountStatus(data, site) {
    if (data === false) {
...
function formatSettings(data, site) {
    if (data && typeof data === 'object') {
        const settings = data;
        return formatKeyValue({
            Site: site,
...
async function runReadAction(options, action) {
    try {
        await action();
    }
    catch (error) {
