...
export async function runBrowserAuth(options) {
    ...
    try {
        ...
        const credentials = 'code' in result
...
async function createCallbackListener(state, timeoutMs) {
    let resolveResult;
    let rejectResult;
    let completed = false;
...
async function handleCallback(request, response, expectedState) {
    ...
    if (!safeEqual(state, expectedState)) {
        ...
        response.end('<h1>WooPayments CLI login failed</h1><p>Invalid login state. Return to the terminal and try again.</p>');
        throw new CliError({
    ...
    if (requestUrl.searchParams.get('success') === '0' || requestUrl.searchParams.get('error')) {
        const message = requestUrl.searchParams.get('error_description') ??
    ...
    if (!code && (!consumerKey || !consumerSecret)) {
        response.writeHead(400, { 'Content-Type': 'text/html; charset=utf-8' });
        response.end('<h1>WooPayments CLI login failed</h1><p>The store did not return a login code.</p>');
        throw new CliError({
    ...
    response.end('<h1>WooPayments CLI connected</h1><p>You can close this tab and return to your terminal.</p>');
    if (code) {
        return { code };
    }
    return {
...
function assertSameOriginAuthorizeUrl(siteUrl, authorizeUrl) {
    let storeOrigin;
    let authorizeOrigin;
    try {
...
function isCredentialResponse(data) {
    return (data !== null &&
        typeof data === 'object' &&
